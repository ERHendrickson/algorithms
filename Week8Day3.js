...
insert(newVal) {
    ...
    while(current){
    //if the newVal exists in the bst we want to return null
    if(newVal == current.data){
        return null;
    }
    //check newVal against root for greater or less and move current accordingly
    if(newVal > current.data){
    ...
    //if current.next encounters null insert newVal in bts
    if(newVal < current.data){
