  ...
  */
   ...
   */
    function mergeCarts(shoppingCart, wishList) {
        //traverse shopping cart and compare each item with wishlist array. if any of the items match add one but not both
...
const mergeCarts2 = (shoppingCart, wishList) => {
    shoppingCart.map(item => {if(!wishList.includes(item)){
        wishList.push(item)
    }})
    return wishList
}
